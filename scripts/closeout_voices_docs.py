...
old_status = """Status: **design authority for issue #9. Slice A (identity and discovery,
...
new_status = """Status: **design and implementation authority for issue #9. Slice A (#10),
...
start_marker = "The shipped registry declares no Perigon source yet, so it plans **four**\n"
end_marker = "Treat every allowance above as external and current. They were read from\nprovider documentation, not measured."
start = s.index(start_marker)
end = s.index(end_marker, start) + len(end_marker)
new_economics = """The hardened shipped registry has **three** active source requests on a
...
s = s[:start] + new_economics + s[end:]

marker = "### 20.7 What was validated, and what was not\n"
start = s.index(marker)
new_tail = """### 20.7 Production validation and remaining provider limits
